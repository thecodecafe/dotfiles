---
name: aidlc
description: Initialize the AWS AI-DLC workflow.
---
...
Treat the complete text supplied with the skill invocation as the initial work request.

```text
$ARGUMENTS
```

- If the block above contains expanded text, use it verbatim as the initial user request.
- If the block is empty or still contains the literal `$ARGUMENTS`, use all text accompanying the `/aidlc` or `$aidlc` invocation instead.
- Preserve the input as one value, including whitespace and line breaks. Do not split it into positional arguments.
- Use this same unmodified value wherever the workflow requires the initial request or its audit entry.
- If no input was supplied, ask the user what they want to work on before continuing the workflow.
...
- `~/.config/.aidlc/aidlc-rules/aws-aidlc-rule-details/` (typical with AI-assisted setup)
- `~/.config/.aidlc-rule-details/` (typical with Cursor, Cline, Claude Code, GitHub Copilot, OpenAI Codex)
- `~/.config/.kiro/aws-aidlc-rule-details/` (typical with Kiro IDE and CLI)
- `~/.config/.amazonq/aws-aidlc-rule-details/` (typical with Amazon Q Developer)
